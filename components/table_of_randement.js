...
import AjouterRandement from './add_randement';
import DeleteRandement from './delete_randement';
import EditeRandement from './edite_randement';


const TableRandement = () => {
  const row = useRouter();
  const { locale } = row;
 ...
 const [columnDefs, setColumnDefs] = useState([
   ...
   {headerName: "Degree de rendement",field: 'degree_rendement', filter: true,  floatingFilter: true, width:700},
   
   {headerName: "Operations", field:"id",width:100,
    cellRenderer:(params)=> 
   <div style={{display: "flex", alignItems:"center", justifyContent:"space-between"}}>
    <EditeRandement p={params}/>
    <DeleteRandement p={params}/>
   </div>},
 ]);
...
const { data, error, isLoading } = useSWR('https://gestion-wheat.vercel.app/randement/', fetcher, { refreshInterval: 1000 })
...
<Typography variant='h6' color='text.secondary' > Tableau des Randement</Typography>
